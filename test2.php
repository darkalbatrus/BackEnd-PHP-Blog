...
if (!$post) {
    echo '<div class="alert alert-danger">مقاله ای یافت نشد ...</div>';
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['postComment'])) {
    ...
    // Insert comment if no validation errors
...
$commentsQuery = $db->prepare("SELECT * FROM comments WHERE post_id = :id AND status = '1'");
$commentsQuery->execute(['id' => $postId]);
?>
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="row justify-content-center">
                    <div class="col">
                        <div class="card">
                            <img src="./uploads/posts/<?= htmlspecialchars($post['image_name']) ?>" class="card-img-top" alt="post-image" />
                            <div class="card-body">
                                <div class="d-flex justify-content-between">
                                    <h5 class="card-title fw-bold"><?= htmlspecialchars($post['title']) ?></h5>
                                    <div>
                                        <span class="badge text-bg-secondary"><?= htmlspecialchars($post['category_title']) ?></span>
                                    </div>
                                </div>
                    ...
                    <div class="col">
                        ...
                        <hr class="mt-4" />
                        <p class="fw-bold fs-6">تعداد کامنت : <?= $commentsQuery->rowCount() ?></p>
                        <?php while ($comment = $commentsQuery->fetch(PDO::FETCH_ASSOC)): ?>
